...
def transcribe_waveform(model: Whisper, enc, waveforms, truncate=False, use_timestamps=getenv('TIMESTAMPS', 0)):
  ...
  def inferloop(ctx: Union[np.ndarray, List[np.ndarray]], encoded_audio):
    pos, next_tokens, sum_logprobs = 0, ctx, np.zeros(ctx.shape[0])
  ...
  all_segments = [[] for _ in waveforms]
  ...
  for curr_frame in range(0, log_spec.shape[-1], FRAMES_PER_SEGMENT):
    ...
    for i, (res, segs, arr) in enumerate(zip(transcriptions, all_segments, ctx)):
