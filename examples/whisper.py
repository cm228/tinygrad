...
def transcribe_waveform(model: Whisper, enc, waveforms, use_beam=False, use_timestamps=False, truncate=False,):
  ...
  def apply_logit_rules(ctx, logits):
    ...
    for i, row in enumerate(ctx):
      ...
      if last == start_tokens[-1]: logits[i, (list(range(start)) if use_timestamps else enc.encode(' ') + [eot])] = -np.inf
        ...
        else: logits[i, :last] = -np.inf # np.r_[:eot, start:last]
  ...
  def process_ctx(toks, seek):
    ...
    if not r: 
      ...
      c = []
    if len(c)>1 and enc.decode(gettexttoks(c[1:]))>0:
  ...
  if use_timestamps: 
    json.dump({'text': transcriptions[0], 'segments': segments[0]}, open('tinygrad_output.json', 'w'))
